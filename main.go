	...
	api := r.Group("/api")
	{
		...
		protected.Use(middleware.AuthMiddleware())
		{
			...
			protected.GET("/users/me",
				middleware.RateLimitUser(rate.Every(time.Minute)/60, 10, "RATE_USER_RPM"),
				handlers.GetMe,
			)
			...
			protected.POST("/chat/sessions",
				middleware.RateLimitUser(rate.Every(time.Minute)/10, 3, "RATE_SESSION_CREATE_RPM"),
			...
			protected.GET("/chat/sessions",
				middleware.RateLimitUser(rate.Every(time.Minute)/30, 5, "RATE_SESSION_LIST_RPM"),
			...
			protected.GET("/chat/sessions/:id/messages",
				middleware.RateLimitUser(rate.Every(time.Minute)/30, 5, "RATE_MESSAGES_RPM"),
			...
			protected.POST("/chat/sessions/:id/messages",
				middleware.RateLimitUser(rate.Every(time.Minute)/20, 3, "RATE_CHAT_SEND_RPM"),
			...
			protected.GET("/subscriptions/status",
				middleware.RateLimitUser(rate.Every(time.Minute)/20, 5, "RATE_SUB_STATUS_RPM"),
			...
			protected.POST("/subscriptions/checkout",
				middleware.RateLimitUser(rate.Every(time.Minute)/5, 2, "RATE_CHECKOUT_RPM"),
			...
			protected.POST("/upload",
				middleware.RateLimitUser(rate.Every(time.Minute)/5, 2, "RATE_UPLOAD_RPM"),
